  ...
  const handleSubmit = async () => {
    ...
    try {
      const packagesPayload = selectedPackages.filter(p => distributions[p._id] > 0).map(pkg => ({
        ...
        total: pkg.retail_price * distributions[pkg._id]
      }));
      ...
      await axios.post(`${API_BASE}/request-inventory`, {
        totalTokens,
        totalAmount: totalCost, // totalCost is already calculated
                              ...
                              <tr key={pkg._id} className="hover:bg-white/5 transition-colors group">
                                ...
                                <td className="px-4 py-3 text-right">
                                  <span className={`font-bold text-sm ${quantity > 0 ? 'text-white' : 'text-slate-600'}`}>
                                    ${(pkg.retail_price * quantity).toFixed(2)}
                                  </span>
                                </td>
                              </tr>
                            );
                    ...
                    <div className="space-y-4 mb-8">
                      {selectedPackages.filter(p => distributions[p._id] > 0).map(pkg => (
                        <div key={pkg._id} className="flex justify-between text-sm animate-slide-in">
                          ...
                          <span className="font-medium text-white">${(pkg.retail_price * distributions[pkg._id]).toLocaleString()}</span>
                        </div>
                      ))}
                      <div className="pt-6 border-t border-white/10 space-y-4">
                        ...
                        <div className="flex justify-between items-baseline">
                          ...
                          <span className="text-4xl font-display font-bold text-orange-500">${totalCost.toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
